	...
	private addAutoComplete() {
		...
		monaco.languages.registerCompletionItemProvider("GherkinLanguage-en", {
			provideCompletionItems: (model, position) => {
				...
				// Récupérer tous les mots du document actuel
				...
				// Récupérer toutes les lignes du document actuel
				const lines = model.getLinesContent();
				const lineSuggestions = lines
					.filter((line) => line.trim().length > 0) // Filtrer les lignes vides
					.map((line) => ({
						label: line,
